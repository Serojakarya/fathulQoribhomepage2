...
function Calendar(isHijr, year, month, firstDay, lang, theme, tmout) {
    ...
    let cd = typeof this == 'object' ? this : window,
        gdate = new Date(),
        hdate = new HijriDate(),
        dispDate, tzOffset = Date.parse('01 Jan 1970'),
        gridAni = 'zoom',
        actTmoId, isDispToday = false,
        isAccOpened = false,
        isAutoNewTheme, isRTL = false,
        ...
        createStyle = function () {
            let stl = document.getElementById('ZulNsCalendarStyle'),
                ct = Calendar.themes,
                ctl = ct.length;
            if (stl) return false;
        ...
        createDates = function () {
            ...
            if (isDispToday) {
                isDispToday = false;
                replaceClass(menuTodayElm, 'w3-transparent', 'w3-button w3-ripple');
                menuTodayElm.disabled = false
            }
            for (let i = 1; i <= ppdr + pcdr + pndr; i++) {
                ...
                let grid = createElm('div', 'w3-cell w3-animate-' + gridAni),
                    ...
                    sde = createElm('div', 'w3-small unbreakable');
                ...
                if (i <= ppdr || ppdr + pcdr < i) {
                    ...
                    if (i % 7 == isFri) grid.className += isToday ? ' w3-teal' : ' w3-text-teal';
                        ...
                        grid.className += ' w3-black w3-round-large w3-btn w3-ripple';
                        grid.style.cursor = 'pointer';
                        addEvt(grid, 'click', onAbout)
                    }
                    if (isToday) {
                        if (actTmoId) {
                            window.clearTimeout(actTmoId);
                            actTmoId = null
                        }
                        isDispToday = true;
                ...
                pde.innerHTML = Calendar.getDigit(pdate);
                sde.innerHTML = Calendar.getDigit(sdate) + ' ' + smsn;
                pdate++;
                if (pdate > pdim) {
                    pdate = 1;
            ...
            let spacer = createElm('div', 'w3-cell-row');
        ...
        onClickMenu = function () {
            if (menuWrapElm.className.indexOf('w3-show') == -1) {
                menuBtnElm.className += ' w3-light-grey';
                menuWrapElm.className += ' w3-show'
            } else hideMenu()
        },
        onChgCalMod = function () {
            cd.setHijriMode(!isHijr);
            applyTodayTmout()
        },
        onFirstDay = function () {
            if (menuFirstDayElm.className.indexOf('expanded') == -1) {
                replaceClass(menuFirstDayElm, 'collapsed', 'expanded');
                isAccOpened = true
            } else {
                replaceClass(menuFirstDayElm, 'expanded', 'collapsed');
                isAccOpened = false
            }
        },
        onSelFirstDay = function (ev) {
            ...
            cd.setFirstDayOfWeek(el.getAttribute('firstday'));
            applyTodayTmout();
        },
        onDispToday = function () {
            cd.today()
        },
        onNewTheme = function () {
            newTheme();
        ...
        onAbout = function () {
            hideMenu();
            aboutTitleElm.innerHTML = 'Kalender Hijriyah/Masehi';
            aboutDateElm.innerHTML = 'Edu StudioNU2020';
            ...
            actTmoId = window.setTimeout(function () {
                aboutElm.style.display = 'none';
                aboutElm.callback = null;
                aboutTitleElm.innerHTML = aboutDateElm.innerHTML = '';
                cd.today()
            }, tmout * 1000)
        },
        onClose = function () {
        ...
        getFixTime = function (t) {
            t -= tzOffset;
            return t - t % 864e5 + 36e5 + tzOffset
        },
        getCurTime = function () {
        ...
        newTheme = function () {
            let ct = Calendar.themes,
                i;
            do i = Math.floor(Math.random() * ct.length); while (ct[i] == theme);
            theme = ct[i]
        },
        applyTheme = function () {
            headerElm.className = headerElm.className.substring(0, headerElm.className.lastIndexOf('w3-')) + 'w3-' + theme
        },
        replaceClass = function (el, dt, sr) {
            el.className = el.className.replace(dt, sr)
        };
    cd.attachTo = function (el) {
        if (el.appendChild && !calElm.parentNode) {
            el.appendChild(calElm);
            onRszWdw();
            return true
        }
        return false
    };
    cd.fireResize = function () {
        onRszWdw()
    };
    cd.getElement = function () {
        return calElm
    };
    ...
    cd.setFullYear = function (y) {
        return cd.resetDate(y)
    };
    ...
    cd.setMonth = function (m) {
        return cd.resetDate(null, m)
    };
    ...
    cd.setTodayTimeout = function (t) {
        t = HijriDate.int(t, tmout);
        if (t >= 10) {
            tmout = t;
            applyTodayTmout();
            return true
    ...
    if (typeof lang == 'string') {
        ...
        if (typeof Calendar.language[lang] != 'object') lang = 'en'
    } else lang = 'en';
    Calendar.lang = lang;
    cd.setTheme(theme);
...
Object.defineProperty(Calendar, 'getDigit', {
    value: function (d) {
        ...
        if (c) return d.toString().replace(/\d(?=[^<>]*(<|$))/g, function ($0) {
            return c[$0]
        });
        return d
    }
});
Object.defineProperty(Calendar, 'themes', {
...
Object.defineProperty(Calendar, 'getVal', {
    value: function (key) {
        return Calendar.language[Calendar.lang][key]
    }
});
Calendar.language = {
    en: {
